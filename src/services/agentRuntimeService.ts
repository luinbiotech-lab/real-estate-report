...
import type { AgentJob, AgentResult, AgentReview, AgentReviewDecision, PropertyRiskAssessment, RenovationAssessment, RiskCheckItem, SpaceType } from '../domain/propertyDataRoom/types';
...
async function executeRiskCompliance(sourceJob: AgentJob) {
  ...
  try {
    ...
    const [documents, candidates, renovations, digitalTwinAssets, spaces] = await Promise.all([
      propertyDataRoomRepository.getDocuments(job.propertyId),
      propertyDataRoomRepository.getVerificationCandidates(job.propertyId),
      propertyDataRoomRepository.getRenovationAssessments(job.propertyId),
      propertyDataRoomRepository.getDigitalTwinAssets(job.propertyId),
      propertyDataRoomRepository.getSpaces(job.propertyId),
    ]);
    ...
    const checks: RiskCheckItem[] = REQUIRED_DOCUMENT_TYPES.map((type) => {
      ...
      return {
        ...
        status: !document ? 'missing' : verified ? 'clear' : 'review',
        detail: !document ? '필수 공적자료가 등록되지 않았습니다.' : verified ? '등록 및 확인 상태입니다.' : `등록됨 · ${document.verificationStatus} 상태로 재확인 필요`,
    ...
    checks.push({
      key: 'verification_candidates', label: '미처리 검증 후보', status: activeCandidates.length ? 'review' : 'clear',
      detail: activeCandidates.length ? `${activeCandidates.length}건의 후보가 승인/거절되지 않았습니다.` : '미처리 검증 후보가 없습니다.',
    });

    checks.push({
      key: 'floor_plan', label: '도면 / 공간 데이터', status: digitalTwinAssets.length || spaces.length ? 'review' : 'missing',
      detail: digitalTwinAssets.length || spaces.length ? `도면·Digital Twin 자산 ${digitalTwinAssets.length}건, 확정 공간 ${spaces.length}건. 실제 치수·구조벽·피난동선은 전문가 확인이 필요합니다.` : '도면 및 공간 데이터가 없어 구조·동선 검토 근거가 부족합니다.',
    });

    checks.push({
      key: 'renovation', label: '리노베이션 검토', status: renovations.length ? 'review' : 'missing',
      detail: renovations.length ? `${renovations.length}건의 리노베이션 검토가 있습니다. 인허가·구조·공사비는 별도 전문가 검토 대상입니다.` : '승인된 리노베이션 검토가 없습니다.',
    });
    ...
    const summary = missingCount
      ? `필수 자료/근거 ${missingCount}개 영역이 부족하고 ${reviewCount}개 영역은 추가 검토가 필요합니다.`
      : reviewCount
        ? `핵심 자료는 연결됐으며 ${reviewCount}개 영역은 전문가·현장 재확인이 필요합니다.`
        : '현재 연결된 자료 기준으로 미해결 체크 항목이 없습니다.';

    return agentOrchestratorService.complete(job, {
      resultType: 'risk_compliance_assessment_candidate',
      payload: {
        assessment: {
          title: '거래·공간·인허가 사전 검토 후보',
          summary,
          checks,
          disclaimer: '본 결과는 자동화된 사전 체크리스트이며 법률·건축·구조·소방·인허가 적합성에 대한 확정 판단이 아닙니다. 최신 공적자료와 관련 전문가의 별도 검토가 필요합니다.',
        },
        adapterVersion: 'risk-compliance-rules-v1',
      },
      confidence: missingCount ? 0.55 : 0.72,
      requiresReview: true,
    });
  } catch (error) {
    await agentOrchestratorService.fail(job, error);
...
async function applyRiskCompliance(result: AgentResult) {
  const raw = result.payload.assessment;
  if (!raw || typeof raw !== 'object') throw new Error('Risk / Compliance Agent 결과 형식이 올바르지 않습니다.');
  const value = raw as Record<string, unknown>;
  const checks = Array.isArray(value.checks) ? value.checks.filter((item): item is RiskCheckItem => Boolean(item && typeof item === 'object' && 'key' in item && 'label' in item && 'status' in item && 'detail' in item)) : [];
  const now = new Date().toISOString();
  const assessment: PropertyRiskAssessment = {
    id: crypto.randomUUID(), propertyId: result.propertyId, title: String(value.title || '사전 리스크 검토'), summary: String(value.summary || ''), checks,
    disclaimer: String(value.disclaimer || '자동 사전검토 결과이며 전문가의 확정 판단이 아닙니다.'), sourceAgentResultId: result.id,
    verificationStatus: 'confirmed', createdAt: now, updatedAt: now,
  };
...
export const agentRuntimeService = {
  ...
  async reviewAndApply(job: AgentJob, review: AgentReview, result: AgentResult, decision: Exclude<AgentReviewDecision, 'pending'>, note = '', reviewedBy?: string) {
    if (result.resultType !== 'renovation_assessment_candidate' && result.resultType !== 'risk_compliance_assessment_candidate') {
      return agentExecutionService.reviewAndApply(job, review, result, decision, note, reviewedBy);
    }
    const reviewed = await agentOrchestratorService.review(job, review, decision, note, reviewedBy);
    if (decision === 'approved') {
      try {
        if (result.resultType === 'renovation_assessment_candidate') await applyRenovation(result);
