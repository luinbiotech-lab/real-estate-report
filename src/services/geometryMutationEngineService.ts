...
export interface WallPartition { u0: number; u1: number; z0: number; z1: number; }
export interface MutatedWallSolid {
  wallSegmentIndex: number; layer: string; start: Point3; end: Point3; thicknessM: number; heightM: number; lengthM: number;
  partitions: WallPartition[]; openingIds: string[]; unionApplied: boolean; miterStrategy: 'endpoint_snap_plus_junction_fill';
}
export interface JunctionFillSolid { junctionId: string; center: Point3; sizeM: number; heightM: number; segmentIndexes: number[]; unionApplied: true; }
export interface SlabSolidWithOpenings {
  roomId: string; elevationM: number; thicknessM: number; shell: Point2[];
...
export interface GeometryMutationValidation {
  ...
  stats: { wallCount: number; wallPartitionCount: number; openingSubtractionCount: number; junctionUnionCount: number; slabCount: number; slabCoreSubtractionCount: number; };
}
export interface GeometryMutationResult {
  schemaVersion: typeof GEOMETRY_MUTATION_SCHEMA_VERSION; engineId: typeof GEOMETRY_MUTATION_ENGINE_ID; sourceAssetId: string; generatedAt: string;
  status: 'mutated_candidate' | 'blocked';
  operations: { wallUnionMiterApplied: boolean; openingSubtractionApplied: boolean; slabCoreSubtractionApplied: boolean; };
  walls: MutatedWallSolid[]; junctionFills: JunctionFillSolid[]; slabs: SlabSolidWithOpenings[];
  validation: GeometryMutationValidation; productionCandidateEligible: boolean; constructionReady: false;
}
...
function snapWalls(asset: DigitalTwinAsset) {
  ...
  for (const junction of eligible) for (const segmentIndex of junction.segmentIndexes) {
    const wall = snapped[segmentIndex]; if (!wall) continue;
    const startDistance = dist(wall.start, junction.pointM); const endDistance = dist(wall.end, junction.pointM);
    if (startDistance <= endDistance) wall.start = { ...junction.pointM }; else wall.end = { ...junction.pointM };
  }
...
export function mutateGeometry(asset: DigitalTwinAsset): GeometryMutationResult {
  ...
  const walls: MutatedWallSolid[] = snapped.map((wall, wallSegmentIndex) => {
    ...
    for (const eligibility of openingEligibility.filter((item) => item.eligible && item.wallSegmentIndex === wallSegmentIndex)) {
      const cut = openingCuts.get(eligibility.candidateId); if (!cut) continue;
      const centerU = wallStation(cut.centerM, wall.start, wall.end);
    ...
    return {
      wallSegmentIndex, layer: wall.layer, start: wall.start, end: wall.end, thicknessM: wall.thicknessM, heightM: wall.heightM, lengthM,
      partitions, openingIds, unionApplied: eligibleJunctions.some((item) => item.segmentIndexes.includes(wallSegmentIndex)), miterStrategy: 'endpoint_snap_plus_junction_fill' as const,
    };
  });

  const junctionFills: JunctionFillSolid[] = eligibleJunctions.flatMap((junction): JunctionFillSolid[] => {
    const participating = junction.segmentIndexes.map((index) => snapped[index]).filter(Boolean); if (!participating.length) return [];
    return [{ junctionId: junction.junctionId, center: junction.pointM, sizeM: Math.max(...participating.map((item) => item.thicknessM)), heightM: Math.min(...participating.map((item) => item.heightM)), segmentIndexes: junction.segmentIndexes, unionApplied: true }];
  });

  const slab = slabGeometryService.build(asset); const coreAlignment = slabCoreAlignmentService.build(asset);
  ...
  const slabs: SlabSolidWithOpenings[] = slab.status === 'ready' ? slab.polygons.map((polygon) => {
    const holes = coreAlignment.filter((item) => item.alignmentStatus === 'aligned').flatMap((core) => {
      ...
      return ring.every((point) => pointInPolygon(point, polygon.polygon)) ? [{ coreId: core.coreId, semantic: core.semantic, ring }] : [];
    });
  ...
  const stats = {
    wallCount: walls.length, wallPartitionCount: walls.reduce((sum, item) => sum + item.partitions.length, 0),
    openingSubtractionCount: walls.reduce((sum, item) => sum + item.openingIds.length, 0), junctionUnionCount: junctionFills.length,
    slabCount: slabs.length, slabCoreSubtractionCount: slabs.reduce((sum, item) => sum + item.holes.length, 0),
  };
  const validation: GeometryMutationValidation = { valid: errors.length === 0, errors, warnings, stats };
  return {
    schemaVersion: GEOMETRY_MUTATION_SCHEMA_VERSION, engineId: GEOMETRY_MUTATION_ENGINE_ID, sourceAssetId: asset.id, generatedAt: new Date().toISOString(),
    status: validation.valid ? 'mutated_candidate' : 'blocked',
    operations: { wallUnionMiterApplied: eligibleJunctions.length > 0, openingSubtractionApplied: stats.openingSubtractionCount > 0, slabCoreSubtractionApplied: stats.slabCoreSubtractionCount > 0 },
    walls, junctionFills, slabs, validation, productionCandidateEligible: validation.valid, constructionReady: false,
  };
}

export const geometryMutationEngineService = { engineId: GEOMETRY_MUTATION_ENGINE_ID, mutate: mutateGeometry };
