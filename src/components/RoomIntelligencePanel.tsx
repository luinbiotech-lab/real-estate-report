...
import type { AgentResult, DigitalTwinAsset, PropertyFacility, PropertyMedia, PropertySpace, RenovationAssessment, RoomConditionHistoryEntry, RoomEvidencePosition, RoomRenovationAssessment, RoomRenovationHistoryEntry, SpaceMediaLink, SpaceRoomLink } from '../domain/propertyDataRoom/types';
import { roomIntelligenceService } from '../services/roomIntelligenceService';
import RoomEvidencePositionPanel from './RoomEvidencePositionPanel';
import RoomExternalViewerPanel from './RoomExternalViewerPanel';
...
export default function RoomIntelligencePanel(props: {
  spaces: PropertySpace[]; assets: DigitalTwinAsset[]; links: SpaceRoomLink[]; media: PropertyMedia[]; spaceMediaLinks: SpaceMediaLink[];
  facilities: PropertyFacility[]; agentResults: AgentResult[]; renovationAssessments: RenovationAssessment[];
  roomRenovationAssessments: RoomRenovationAssessment[]; roomEvidencePositions: RoomEvidencePosition[]; roomConditionHistory: RoomConditionHistoryEntry[]; roomRenovationHistory: RoomRenovationHistoryEntry[];
  ...
  const roomPositions = view ? props.roomEvidencePositions.filter((item) => item.spaceId === view.space.id && item.roomCandidateId === view.room.roomCandidate.id) : [];

  return <section style={{ background: '#fff', border: '1px solid #d9e0e8', borderRadius: 12, padding: 20, marginBottom: 20 }}>
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap' }}>
      <div><h2 style={{ margin: 0 }}>Room Intelligence View</h2><p style={{ color: '#667085', margin: '6px 0 0' }}>승인된 Interior ↔ 3D Room 연결을 기준으로 사진·설비 위치, 상태 이력, 리노베이션 이력과 외부 검토 패키지까지 공간 단위로 묶습니다.</p></div>
    ...
    {view && <>
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(240px,.7fr) minmax(0,2fr)', gap: 14, marginTop: 16 }}>
        <div>
          <Select fullWidth size="small" value={view.link.id} onChange={(event) => setSelectedId(event.target.value)}>{views.map((item) => <MenuItem key={item.link.id} value={item.link.id}>{item.space.floor || '층 미확인'} · {item.space.name} → {item.room.roomName}</MenuItem>)}</Select>
          <div style={{ marginTop: 12 }}><RoomShape points={view.room.roomCandidate.points} /></div>
          <div style={{ display: 'grid', gap: 7, marginTop: 12 }}><div><small style={{ color: '#667085' }}>Interior Space</small><strong style={{ display: 'block' }}>{view.space.name}</strong></div><div><small style={{ color: '#667085' }}>3D Room</small><strong style={{ display: 'block' }}>{view.room.roomName}</strong></div><div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}><Chip size="small" label={view.space.spaceType} /><Chip size="small" variant="outlined" label={view.space.floor || view.room.floor || '층 미확인'} />{view.room.roomCandidate.areaSqmCandidate && <Chip size="small" variant="outlined" label={`${view.room.roomCandidate.areaSqmCandidate.toFixed(1)}㎡ 후보`} />}</div></div>
        </div>

        <div style={{ display: 'grid', gap: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5,minmax(100px,1fr))', gap: 8 }}>{[['사진', view.evidence.directMediaCount], ['설비', view.evidence.directFacilityCount], ['위치 승인', roomPositions.filter((item) => item.decision === 'approved').length], ['Vision 근거', view.evidence.visionResultCount], ['연결 신뢰도', `${Math.round((view.link.confidence ?? 0) * 100)}%`]].map(([label, value]) => <div key={String(label)} style={{ padding: 12, borderRadius: 10, background: '#f7f9fb', border: '1px solid #e7ebf0' }}><small style={{ color: '#667085' }}>{label}</small><strong style={{ display: 'block', fontSize: 20, marginTop: 4 }}>{value}</strong></div>)}</div>

          <div><h3 style={{ margin: '0 0 9px' }}>3D Room Evidence Overlay</h3><RoomIntelligence3DOverlay points={view.room.roomCandidate.points} media={view.media} facilities={view.facilities} positions={roomPositions} ceilingHeightM={view.space.ceilingHeightM} /></div>

          <RoomEvidencePositionPanel view={view} positions={props.roomEvidencePositions} onSaved={props.onSaved} />

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}><div style={{ border: '1px solid #e3e8ef', borderRadius: 10, padding: 14 }}><strong>현재 상태</strong><p style={{ color: '#475467', marginBottom: 0 }}>{view.space.currentCondition || '확인 필요'}</p></div><div style={{ border: '1px solid #e3e8ef', borderRadius: 10, padding: 14 }}><strong>추천 용도</strong><p style={{ color: '#475467', marginBottom: 0 }}>{view.space.recommendedUse || '판단 보류'}</p></div></div>
          ...
          <RoomHistoryPanel view={view} conditionHistory={props.roomConditionHistory} renovationHistory={props.roomRenovationHistory} onSaved={props.onSaved} />
          <RoomExternalViewerPanel view={view} positions={props.roomEvidencePositions} conditionHistory={props.roomConditionHistory} renovationAssessments={props.roomRenovationAssessments} renovationHistory={props.roomRenovationHistory} />
      ...
      <Alert severity="warning" sx={{ mt: 1.5 }}>사진·설비 위치는 Human Review된 room-local 좌표이며 측량/BIM 기준좌표가 아닙니다. Room Renovation도 Human Review를 통과하기 전까지 확정 공사 범위로 사용하지 않습니다.</Alert>
    </>}
  </section>;
}
