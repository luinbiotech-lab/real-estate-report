...
export function DaonDetail7PageMaster({ snapshot, model }: { snapshot: ReportSnapshot; model: ProfessionalReportViewModel }) {
  const visibleMedia = model.media.items.filter((item) => item.url && item.category !== 'interior');
  ...
  return <div className="daon-detail-master" data-template-id={DAON_DETAIL_MASTER_TEMPLATE_ID} data-template-version={DAON_DETAIL_MASTER_TEMPLATE_VERSION}>
    <Page page={1} eyebrow="WHY THIS ASSET" title={model.identity.name.display} subtitle="매입 관점에서 보는 핵심 제안" snapshot={snapshot} model={model}>
      <div className="dd-p1-top">
        <div className="dd-p1-hero"><ImageSlot src={hero} caption="대표 외관" /><div className="dd-hero-copy"><b>바로 쓰고, 바꿔 쓰고,<br/>오래 볼 수 있는 코너 자산</b><span>실사용 + 리포지셔닝 + 토지가치</span></div></div>
        <div className="dd-p1-side">
          ...
          <NumberedPoint index={2} title="코너 대지의 존재감" copy={display(model.land.roadCondition, '도로·코너 조건 현장 확인 필요')} />
        </div>
      </div>
    ...
    <Page page={2} eyebrow="PROPERTY PROFILE" title="PROPERTY PROFILE" subtitle="공적자료와 현장 이용을 함께 보는 자산 구성" snapshot={snapshot} model={model}>
      ...
      <div className="dd-photo-three"><ImageSlot src={imageAt(0)} caption="인접 주거환경" /><ImageSlot src={imageAt(1)} caption="생활권·주변 상권" /><ImageSlot src={imageAt(2)} caption="인근 코너형 상업·업무 건물" /></div>
    ...
    <Page page={3} eyebrow="LOCATION LEVERAGE" title="LOCATION LEVERAGE" subtitle="코너 입지와 생활권 수요를 매입 가치로 연결하는 방법" snapshot={snapshot} model={model}>
      <SectionBar right="LOCATION MAP">입지의 핵심 구조</SectionBar>
      <div className="dd-location-grid"><div className="dd-map-wrap"><ImageSlot src={map} caption="입지 지도" className="map" /></div><div className="dd-location-cards"><GoldCard title="보이는 코너" copy={display(model.land.roadCondition, '양면 도로·파사드 노출·출입 동선 확인 필요')} /><GoldCard title="생활권 안의 목적형 자산" copy={display(model.location.locationAnalysis, '생활·상권·배후수요 분석 데이터 확인 필요')} /><GoldCard title="실사용 매수자에게 맞는 규모" copy={`${display(model.land.landAreaPyeong)} 토지 규모와 기존 건물 활용성을 함께 검토합니다.`} /></div></div>
      <div className="dd-photo-three"><ImageSlot src={imageAt(0)} caption="인접 주거환경" /><ImageSlot src={imageAt(1)} caption="생활권과 다양한 상권" /><ImageSlot src={imageAt(2)} caption="가시성 높은 코너 입지" /></div>
    ...
    <Page page={4} eyebrow="EXCLUSIVE SALE REPORT | 04" title="MARKET POSITION & PRICE RATIONALE" subtitle="실거래 사례 속에서 본건의 토지 평당을 읽는 방법" snapshot={snapshot} model={model}>
      <SectionBar>가격 포지션</SectionBar>
      <div className="dd-price-position"><div><small>본건 토지 평당</small><strong>{display(model.pricing.landUnitPrice)}</strong></div><div><b>가격을 설득하는 핵심은 “평균보다 싸다”가 아닙니다.</b><p>코너 입지, 명도·실사용 가능성, 기존 건물의 활용성과 같은 개별 조건을 확인된 비교사례와 함께 설명합니다.</p></div></div>
      <SectionBar right="단위: 입력자료 기준">비교 사례</SectionBar>
      <div className="dd-comparables">{model.investment.nearbyTransactions.value ? <p>{model.investment.nearbyTransactions.display}</p> : <div><b>비교 거래 데이터 미연결</b><span>04_실거래_비교 시트 또는 검증된 거래자료 연결 후 표와 가격포지션을 확정합니다.</span></div>}</div>
      ...
      <div className="dd-reason-three"><GoldCard title="총액과 규모의 균형" copy={`${display(model.land.landAreaPyeong)} 토지와 기존 건물을 함께 확보하는 구조를 검토합니다.`} /><GoldCard title="프리미엄의 근거" copy={display(model.land.roadCondition)} /><GoldCard title="리스크의 투명성" copy={risks[0] || '공적자료·현장·권리관계를 확인해 가격 리스크를 투명하게 제시합니다.'} /></div>
    </Page>

    <Page page={5} eyebrow="DA:ON ASSET" title="VALUE CREATION SCENARIOS" subtitle="매입 후 바로 실행할 수 있는 세 가지 방향" snapshot={snapshot} model={model}>
      <SectionBar right="VALUE CREATION ROADMAP">가치 확장 로드맵</SectionBar>
      <div className="dd-roadmap"><NumberedPoint index={1} title="즉시 사용" copy={display(model.pricing.occupancyStatus)} /><NumberedPoint index={2} title="리포지셔닝" copy={display(model.investment.features, '파사드·내부동선·공간 재구성 검토')} /><NumberedPoint index={3} title="중장기 검토" copy={display(model.investment.developmentPlan, '리노베이션 또는 신축 여부 검토')} /></div>
    ...
    <Page page={6} eyebrow="BUILDING & DEVELOPMENT REVIEW" title="BUILDING & DEVELOPMENT REVIEW" subtitle="기존 건물의 활용성과 중장기 개발 검토 포인트" snapshot={snapshot} model={model}>
      ...
      <div className="dd-photo-three"><ImageSlot src={imageAt(0)} caption="본건 인근 코너 건물 전경" /><ImageSlot src={imageAt(1)} caption="주변 랜드마크·배후수요" /><ImageSlot src={imageAt(2)} caption="주변 도로·생활권 전경" /></div>
      <SectionBar>건물 지표</SectionBar>
      <div className="dd-metrics"><div><span>●</span><small>단순 건폐율</small><strong>{display(model.building.buildingCoverageRate)}</strong></div><div><span>●</span><small>단순 용적률</small><strong>{display(model.building.floorAreaRatio)}</strong></div><div><span>●</span><small>사용승인</small><strong>{display(model.building.completionDate)}</strong></div></div>
