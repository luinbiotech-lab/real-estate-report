  ...
  return <div className="report-v2 document">
    ...
    <article className="a4 portrait report-page report-page-two">
      ...
      <section className="analysis-layout">
        ...
        <div className="analysis-summary"><TextBlock title="종합 의견" value={p.overallOpinion} accent limit={4} />{p.additionalImages.length > 0 && <div className="analysis-thumbs">{p.additionalImages.slice(0, 2).map((src, index) => <Photo key={index} src={src} label={`현장사진 ${index + 1}`} />)}</div>}</div>
      </section>
...
function Proposal({ property: p, settings: s, layout }: { property: Property; settings: Settings; layout: 'portrait' | 'landscape' }) {
  const unitPrice = pricePerPyeong(p);
  ...
  return <div className={`proposal-v2 document proposal-${layout}`}><article className={`a4 ${layout} proposal-sheet`}>
    ...
    <section className="proposal-showcase"><Photo src={p.mainImage} label="대표사진" className="proposal-main-photo" /><div className="proposal-points"><p className="mini-label">KEY HIGHLIGHTS</p><h2>핵심 포인트</h2><ol>{corePoints(p).map((point, index) => <li key={index}><span>{String(index + 1).padStart(2, '0')}</span><b>{point}</b></li>)}</ol></div></section>
