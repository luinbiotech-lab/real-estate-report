...
export default function RoomTwinOperationsPage() {
  ...
  useEffect(() => { (async () => { try { const list = await propertyRepository.getAll(); setProperties(list); const first = list[0]?.id || ''; setPropertyId(first); if (first) await load(first); } catch (reason) { setError(reason instanceof Error ? reason.message : 'Room Twin Operations를 불러오지 못했습니다.'); } finally { setLoading(false); } })(); }, []);
