...
const explicitImportKeys = (row: ImportRow): (keyof Property)[] => {
  const keys = Object.entries(row.raw).flatMap(([header, rawValue]) => {
...
export default function ExcelImport() {
  ...
  const run = async (selectedOnly = false) => { if (!job || job.running) return; await mapService.refreshProviderStatus(); pauseRef.current = false; await runImportQueue(job, commit, () => pauseRef.current, selectedOnly); };
  ...
  return <><header className="page-header compact"><div><Button startIcon={<ArrowBackRounded />} onClick={() => nav('/')}>목록으로</Button><h1>Excel 가져오기</h1><p>검증 → 자동조회 → 주소·POI 검토 → 확정 저장 순서로 안전하게 처리합니다.</p></div><Button variant="outlined" startIcon={<DownloadRounded />} onClick={downloadTemplate}>표준 양식 다운로드</Button></header>
    ...
    <Dialog open={!!review && !!reviewRow} onClose={() => setReview(undefined)} fullWidth maxWidth="md"><DialogTitle>{review?.type === 'address' ? '주소 후보 검토' : 'POI 후보 검토'}</DialogTitle><DialogContent>{review?.type === 'address' ? <div className="address-review"><p>입력 주소: <b>{String(reviewRow?.normalizedProperty.address || '-')}</b></p>{reviewRow?.addressCandidates?.map((candidate, index) => <div key={index}><div><b>{candidate.roadAddress || candidate.jibunAddress}</b><small>{candidate.jibunAddress}<br />{candidate.latitude}, {candidate.longitude} · {candidate.provider}</small></div><Button onClick={() => { if (reviewRow) updateRow(reviewRow.rowId, (row) => selectAddressCandidate(row, index)); setReview(undefined); }}>이 주소 사용</Button></div>)}</div> : <div className="poi-review">{reviewRow?.poiCandidates?.map((poi) => <div key={poi.id}><Checkbox checked={poi.selected} onChange={(event) => reviewRow && updateRow(reviewRow.rowId, (row) => ({ ...row, poiCandidates: row.poiCandidates?.map((item) => item.id === poi.id ? { ...item, selected: event.target.checked } : item) }))} /><div><b>{poi.name}</b><small>{poi.distanceMeters ?? '-'}m · {poi.category}</small></div><TextField select size="small" value={poi.briefingCategory} onChange={(event) => reviewRow && updateRow(reviewRow.rowId, (row) => ({ ...row, poiCandidates: row.poiCandidates?.map((item) => item.id === poi.id ? { ...item, briefingCategory: event.target.value as BriefingCategory } : item) }))}>{BRIEFING_CATEGORIES.map((category) => <MenuItem key={category.key} value={category.key}>{category.label}</MenuItem>)}</TextField></div>)}</div>}</DialogContent><DialogActions><Button onClick={() => setReview(undefined)}>닫기</Button></DialogActions></Dialog>
  </>;
}
