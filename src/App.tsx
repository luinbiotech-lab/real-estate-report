...
export default function App() {
  ...
  useEffect(() => { (async () => {
    ...
    const currentBangbae = await propertyRepository.getById(bangbae81511.id);
  ...
  return <ThemeProvider theme={theme}><ProductionAuthGate><Routes><Route element={<Layout />}><Route index element={<PropertyList />} /><Route path="control-center" element={<AgentControlCenterPage />} /><Route path="agents/:agentId" element={<AgentWorkspacePage />} /><Route path="property/new" element={<PropertyForm settings={settings} />} /><Route path="property/:id" element={<PropertyHubPage />} /><Route path="property/:id/data-room" element={<PropertyDataRoomPage />} /><Route path="property/:id/edit" element={<PropertyForm settings={settings} />} /><Route path="import" element={<ExcelImport />} /><Route path="bulk-intake" element={<BulkIntakePage />} /><Route path="readiness" element={<PropertyReadinessCenterPage />} /><Route path="interior" element={<InteriorWorkspacePage />} /><Route path="room-ops" element={<RoomTwinOperationsPage />} /><Route path="spatial" element={<SpatialWorkspacePage />} /><Route path="digital-twin-intake" element={<DigitalTwinIntakePage />} /><Route path="digital-twin" element={<DigitalTwinWorkspacePage />} /><Route path="external-shares" element={<ExternalShareCenterPage />} /><Route path="income" element={<RentalIncomeWorkspacePage />} /><Route path="review-history" element={<ReviewHistoryPage />} /><Route path="access" element={<AccessManagementPage />} /><Route path="risk" element={<RiskWorkspacePage />} /><Route path="agents" element={<AgentOpsPage />} /><Route path="report-history" element={<ReportHistoryPage />} /><Route path="backup" element={<DataBackupCenterPage />} /><Route path="migration-readiness" element={<RemoteMigrationReadinessPage settings={settings} />} /><Route path="settings" element={<SettingsPage settings={settings} onSave={setSettings} />} /></Route><Route path="document/:kind/:id" element={<DocumentPreview />} /><Route path="properties/:id/briefing" element={<PropertyBriefingPage settings={settings} />} /><Route path="professional-report/snapshot/:snapshotId" element={<ProfessionalReportSnapshotPage />} /><Route path="*" element={<Navigate to="/" />} /></Routes></ProductionAuthGate></ThemeProvider>;
}