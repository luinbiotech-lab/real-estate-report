...
if (!twinPackage.includes('wallThicknessReviewed') || !twinPackage.includes('floorPlacementVerified') || !twinPackage.includes('openingCutsPrepared') || !twinPackage.includes('verticalCoreReviewed')) throw new Error('Twin handoff readiness is missing wall/floor/opening/core signals.');
if (!twinPackage.includes('verticalCoreNodes') || !twinPackage.includes('productionMeshReady: false')) throw new Error('Twin handoff vertical core/safety payload is incomplete.');
...
for (const component of ['WallModelPanel', 'OpeningCutPanel', 'FloorPlacementPanel', 'VerticalCorePanel', 'BuildingStackPanel']) if (!twinPage.includes(component)) throw new Error(`Digital Twin Workspace missing ${component}.`);

console.log('DA:ON wall + opening cut + slab + vertical core + multi-floor building integrity: PASS');
