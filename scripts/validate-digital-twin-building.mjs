...
if (!twinPackage.includes('wallThicknessReviewed') || !twinPackage.includes('floorPlacementVerified') || !twinPackage.includes('slabGeometryPrepared') || !twinPackage.includes('openingCutsPrepared') || !twinPackage.includes('verticalCoreReviewed')) throw new Error('Twin handoff readiness is missing wall/floor/slab/opening/core signals.');
if (!twinPackage.includes('slabGeometry') || !twinPackage.includes('verticalCoreNodes') || !twinPackage.includes('productionMeshReady: false')) throw new Error('Twin handoff slab/core/safety payload is incomplete.');
...
for (const component of ['WallModelPanel', 'OpeningCutPanel', 'FloorPlacementPanel', 'SlabGeometryPanel', 'VerticalCorePanel', 'BuildingStackPanel']) if (!twinPage.includes(component)) throw new Error(`Digital Twin Workspace missing ${component}.`);

console.log('DA:ON wall + opening cut + slab geometry + vertical core + multi-floor building integrity: PASS');
