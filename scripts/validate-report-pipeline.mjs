...
if (!text.professionalMaster.includes('DaonReportOpeningPage') || !text.professionalMaster.includes('DaonDetail7PageMaster') || !text.professionalMaster.includes('DaonReportClosingPage')) throw new Error('DAON PROFESSIONAL MASTER는 OPENING + DETAIL + CLOSING 구조를 유지해야 합니다.');
...
if (!text.bangbaeSeed.includes("verificationStatus: 'imported'") || !text.bangbaeSeed.includes('const existingFloor = spaces.some') || !text.bangbaeSeed.includes('if (existingFloor) continue') || !text.bangbaeSeed.includes('if (!sources.some((item) => item.id === COMPARABLE_SOURCE_ID))')) throw new Error('방배동 bootstrap은 imported provenance를 보존하고 기존 층·비교거래를 덮어쓰지 않아야 합니다.');
if (!text.bangbaeSeed.includes("{ floor: '3F'") || !text.bangbaeSeed.includes("{ floor: 'B1'") || !text.bangbaeSeed.includes("label: '방배동 448-37'")) throw new Error('방배동 층별 4개와 비교거래 6건 seed 데이터가 누락되었습니다.');
...
if (!text.spaceOverviewPanel.includes('OFFICIAL FLOOR STRUCTURE') || !text.spaceOverviewPanel.includes('층별 구성 · Data Room') || !text.spaceOverviewPanel.includes('imported 상태를 임의로 verified로 승격하지 않습니다.')) throw new Error('층별 구성 패널은 공부상 층·면적·검증상태·출처를 표시하고 imported provenance를 명확히 유지해야 합니다.');
