...
try {
  await verifyPage(page, '/control-center', ['Agent Control Center', 'A0 · INTEGRATOR', 'Report Agent', 'Agent Workspace'], 'agent-control-center');
  await verifyPage(page, '/agents/interior-vision', ['A4 · ISOLATED AGENT WORKSPACE', 'Interior Vision Agent', 'Owned Modules', 'Agent Contract', 'Upstream', 'Downstream'], 'agent-workspace-interior-vision');
  await verifyPage(page, '/agents/digital-twin', ['A7 · ISOLATED AGENT WORKSPACE', 'Digital Twin Agent', 'Owned Modules', 'Agent Contract'], 'agent-workspace-digital-twin');
  ...
  console.log('Rendered A0-A11 modular agent navigation + isolated workspaces + existing domain workspaces smoke QA: PASS');
} catch (error) {
  await page.screenshot({ path: `${ARTIFACT_DIR}/failure.png`, fullPage: true });
  console.error('Rendered modular agent architecture smoke QA: FAIL');
  console.error(error);
  throw error;
} finally {
  await browser.close();
