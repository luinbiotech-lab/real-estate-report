...
try {
  ...
  console.log('Rendered Interior room positioning/history/viewer + integrated Room Twin Operations + Spatial + Digital Twin + Agent + Risk smoke QA: PASS');
} catch (error) {
  await page.screenshot({ path: `${ARTIFACT_DIR}/failure.png`, fullPage: true });
  console.error('Rendered room positioning/history/viewer + Room Twin Operations smoke QA: FAIL');
  console.error(error);
  throw error;
} finally {
  await browser.close();
