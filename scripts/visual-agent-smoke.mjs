...
try {
  await verifyPage(page, '/interior', ['Interior Workspace', 'Interior ↔ 3D Room Linking', '추천 연결 후보', 'Room Intelligence View', 'Intelligent rooms', 'Vision 분석 기록', '승인된 설비 인벤토리', '브라우저 로컬 픽셀 분석'], 'interior-workspace');
  ...
  console.log('Rendered Interior room intelligence + 3D room linking + Spatial + Digital Twin release collaboration + Agent + Risk smoke QA: PASS');
} catch (error) {
  await page.screenshot({ path: `${ARTIFACT_DIR}/failure.png`, fullPage: true });
  console.error('Rendered Interior room intelligence + 3D room linking + Spatial + Digital Twin release collaboration + Agent + Risk smoke QA: FAIL');
  console.error(error);
  throw error;
} finally {
  await browser.close();
