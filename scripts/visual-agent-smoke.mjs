...
try {
  ...
  await verifyPage(page, '/digital-twin', ['Digital Twin Workspace', 'Geometry', '축척', '높이', '공간 경계', '문·창 연결', '개구부 치수', 'Twin 후보 갱신', 'handoff package', 'Spatial Workspace에서 PDF/DWG/DXF 도면 원본을 먼저 등록하세요.'], 'digital-twin-workspace');
  ...
  console.log('Rendered Interior + Spatial + Digital Twin shell + Agent + Risk smoke QA: PASS');
} catch (error) {
  await page.screenshot({ path: `${ARTIFACT_DIR}/failure.png`, fullPage: true });
  console.error('Rendered Interior + Spatial + Digital Twin shell + Agent + Risk smoke QA: FAIL');
  console.error(error);
  throw error;
} finally {
  await browser.close();
