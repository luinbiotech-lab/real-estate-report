...
try {
  ...
  await verifyPage(page, '/digital-twin', ['Digital Twin Workspace', 'Geometry 추출', '축척 검증', '높이 검증', '공간 경계 승인', '3D', 'DWG는 별도 변환기'], 'digital-twin-workspace');
  ...
  console.log('Rendered Interior + Spatial + Room Topology + 3D Preparation + Agent + Risk smoke QA: PASS');
} catch (error) {
  await page.screenshot({ path: `${ARTIFACT_DIR}/failure.png`, fullPage: true });
  console.error('Rendered Interior + Spatial + Room Topology + 3D Preparation + Agent + Risk smoke QA: FAIL');
  console.error(error);
  throw error;
} finally {
  await browser.close();
