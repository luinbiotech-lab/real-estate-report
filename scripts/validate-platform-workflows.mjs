...
const roomOpsPage = readFileSync('src/pages/RoomTwinOperationsPage.tsx', 'utf8');
const interiorWorkspacePage = readFileSync('src/pages/InteriorWorkspacePage.tsx', 'utf8');
for (const [label, source] of [['Room Twin Operations', roomOpsPage], ['Interior Workspace', interiorWorkspacePage]]) {
  for (const required of ['useSearchParams', "searchParams.get('propertyId')", 'requestedExists', 'setSearchParams({ propertyId: nextId })']) {
    if (!source.includes(required)) throw new Error(`${label} 물건 컨텍스트 유지 계약 누락: ${required}`);
  }
}
...
if (!text.externalShareProvider.includes('실제 OWNER issue/list/revoke browser acceptance는 아직 필요합니다.')) throw new Error('Remote Provider는 backend 연결과 operator acceptance를 분리해야 합니다.');
...
if (!text.externalShareCenter.includes('LOCAL / OFFLINE') || !text.externalShareCenter.includes('REMOTE / PUBLIC') || !text.externalShareCenter.includes('REMOTE / PUBLIC backend는 CONNECTED입니다.')) throw new Error('외부 공유 센터는 Local/Remote Provider 상태와 backend/operator acceptance를 구분해 표시해야 합니다.');
...
for (const marker of ["backendState: 'ready'", "operatorAcceptance: 'required'", 'BACKEND CONNECTED', 'remoteOperatorAcceptance']) {
  const target = marker === 'BACKEND CONNECTED' || marker === 'remoteOperatorAcceptance' ? text.externalShareCenter : text.externalShareProvider;
  if (!target.includes(marker)) throw new Error(`Remote share backend/operator acceptance 계약 누락: ${marker}`);
}
