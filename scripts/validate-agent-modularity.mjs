...
for (const name of ['IntegratorReadPort', 'AgentRuntimePort', 'RenovationAgentPort', 'RiskComplianceAgentPort', 'integratorReadPort', 'agentRuntimePort', 'renovationAgentPort', 'riskComplianceAgentPort']) if (!ports.includes(name)) throw new Error(`Narrow agent data port missing ${name}`);
if (orchestrator.includes("../repositories/propertyDataRoomRepository") || runtime.includes("../repositories/propertyDataRoomRepository")) throw new Error('Agent runtime/orchestrator must not directly import the shared repository.');
if (!orchestrator.includes("../agents/agentDataPorts") || !runtime.includes("../agents/agentDataPorts")) throw new Error('Agent runtime/orchestrator must use agent data ports.');
...
console.log('DA:ON A0-A11 modular registry + contracts + narrow data ports + A0 read isolation + write isolation + readiness status + integrator routing: PASS');
