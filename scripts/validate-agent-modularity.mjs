...
for (const name of ['AgentRuntimePort', 'RenovationAgentPort', 'RiskComplianceAgentPort', 'agentRuntimePort', 'renovationAgentPort', 'riskComplianceAgentPort']) if (!ports.includes(name)) throw new Error(`Narrow agent data port missing ${name}`);
...
console.log('DA:ON A0-A11 modular registry + contracts + narrow data ports + write isolation + readiness status + integrator routing: PASS');
