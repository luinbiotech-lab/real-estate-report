...
for (const marker of [
  ...
  'remoteDataGateway.getProperty(propertyId)',
  'existingRemoteProperties.length',
  '기존 원격 Property overwrite는 별도 동기화 워크플로에서만 허용합니다.',
  'const localAssetBlobs = new Map<string, Blob>()',
  '첫 원격 write 전 preflight에서 local Blob을 찾을 수 없습니다.',
  'localAssetBlobs.get',
